...
gradient_r <- function(beta, y, x){
    ...
    for (i in 1:len) {
        a <- c(1,x[i]) *(y[i] - 
        exp(beta[1] + beta[2] * x[i]) / (1 + exp(beta[1] + beta[2] * x[i])))
        gradient <- gradient + a
    }
    gradient
}
...
return wrap(sum);
"
gradient_c <- cxxfunction(signature(beta = "numeric", x = "numeric",
    y = "numeric"), body=gradient_code, plugin="RcppArmadillo")
```
...
#to test if the log likelihood function in R and Rcpp produce the same result
...
all.equal(gradient_r(x=x, y=y, beta=beta), c(gradient_c(x=x, y=y, beta = beta)))
```
