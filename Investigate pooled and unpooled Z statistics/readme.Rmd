...
for(i in 1:N){
    ...
    stat[i,2]   <- (p1hat-p2hat)/sqrt(p1hat*(1-p1hat)/n+p2hat*(1-p2hat))
}
...
plotalpha <- function(n=10){
    ...
    #(a)look at alpha level for different true values of p
    ...
    for(i in 1:m){
        ...
        for(j in 1:N){
            ...
            #compute the pooled and unpooled z statistics
            stat[j,1]   <- ifelse(p1hat==p2hat,0,(p1hat-p2hat)/sqrt(2*phat*(1-phat)/n))
            stat[j,2]   <- ifelse(p1hat==p2hat,0,(p1hat-p2hat)/sqrt(p1hat*(1-p1hat)/n+p2hat*(1-p2hat)/n))    
        }
    ...
    ggplot(alphasnew,aes(x=p,y=alphas,color=indicator)) + 
#       geom_point() + 
        geom_line() + 
        labs(title=paste0('Observed alpha levels for two Z statistics \n against different true values of p given n=',n),x='True values of p',y='Observed significance levels') +
        scale_color_discrete(name='Test Statistic Z', labels=c('pooled','unpooled')) +
        theme_bw()
}
...
plotpow <- function(p1=0.5,p2=0.4){
    ...
    #plot the needed plot
    ggplot(powersnew,aes(x=n,y=powers,color=indicator)) + 
 #      geom_point() + 
        geom_line() + 
        labs(title=paste0('Powers for two Z statistics against different values \n of n given p1=',p1,' and p2=',p2),x='n',y='Power') +
        scale_color_discrete(name='Test Statistic Z', labels=c('pooled','unpooled')) +
        theme_bw()
}
